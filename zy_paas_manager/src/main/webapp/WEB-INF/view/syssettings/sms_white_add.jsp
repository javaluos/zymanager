		...
		style="padding-bottom: -20px; margin-bottom: 10px;">
		    ...
		    <table class="noborder" style="margin-left: 20px;width:800px">
				<tbody>
					...
					<tr class="noline">
						<td class="fr" style="width: 80px; margin-left: 20px"><span style="color: red;">*</span>白名单号码:</td>
						<td class="online" style="width: 400px">
...
<script type="text/javascript">
	function dosubmit(){
		...
		if(mobile==null||mobile==''){
			layer.tips("请输入白名单号码!", $('#mobile'));
			return ;
		}
        var errmsg = checkphone(mobile);
        if(errmsg != null && errmsg != ''){
            $("#errmsg").text("号码" + errmsg + "已加入黑名单");
            return;
        }
		$.ajax({
			type: "POST",
	   		url: "/sms_white_list/do_add",
	   		data: {"businessname" : businessname, "mobile" : mobile,"remark":remark},
	   		dataType: "json",
		   	success: function(data){
		   		if(data>0){
		   			layer.alert("添加白名单号码成功,总计添加"+data+"个！");
	...
	function checkphone(mobile){
	    ...
	    $.ajax({
	        async: false,
	        type: "POST",
            url: "/sms_white_list/check_phone",
            data: {"mobiles" : mobile},
            dataType: "text",
            success: function(data){
                phones = data;
