...
<table class="table table-hover">
	...
	<tr>
		...
		<th width="80" style="text-align: center;">被叫显号</th>
	...
	<div class="panel-foot text-center pagereq">
		<ul class="pagination">
			...
			<li style="border: none;">&nbsp;<span style="color:red;">${pgdata.page_num}</span> / <span style="color:red;">${pgdata.total_page}</span> 页&nbsp;</li>
		</ul>
		<ul class="pagination">
		...
		<ul class="pagination"> 
		   <c:if test="${pgdata.page_num<pgdata.total_page}">
			  <li><a href="javascript:;" _pgnum="${pgdata.page_num+1 }">下一页</a></li>
			</c:if>
			<c:if test="${pgdata.page_num>=pgdata.total_page}">
			  <li><a href="javascript:;" _pgnum="${pgdata.total_page }">下一页</a></li>
			</c:if>
		</ul>
		<ul class="pagination">
			...
			<li><button type="button" class="button bg-main" id="pggobtn"
			    ...
			    <input type="hidden" id="maxpg" value="${pgdata.total_page}">
		    </li>   
		</ul>
	</div>
</div>
