			...
			<thead>
				<tr role="row">
					<th tabindex="0" aria-controls="DataTables_Table_0"
						rowspan="1" colspan="1" style="min-width:70px">通道名称</th>
					<th tabindex="0" aria-controls="DataTables_Table_0"
			...
			<tbody>
				<c:if test="${not empty pgdata.data }">
				    <c:choose>
				        <c:when test="${size > 1}">
				            <tr>
                                <td rowspan="${pgdata.total + 1}" style="vertical-align:middle;text-align:center">${channelName}</td>
                                <td rowspan="${pgdata.total + 1}" style="vertical-align:middle;text-align:center">${channelId}</td>
