		...
		<table
			...
			<thead>
				<tr role="row">
					...
					<th tabindex="0" aria-controls="DataTables_Table_0"
						rowspan="1" colspan="1" style="min-width:40px">类型</th>
					<th tabindex="0" aria-controls="DataTables_Table_0"
				...
				<c:if test="${not empty pgdata.data }">
					<c:forEach var="dt" items="${pgdata.data}" varStatus="dtindex">
						<tr class="gradeC odd">
							...
							<td>
							    <c:if test="${dt.smsCategory == 8 }">
							                 通知
							    </c:if>
							    <c:if test="${dt.smsCategory == 9 }">
                                             验证码
                                </c:if>
                                 <c:if test="${dt.smsCategory == 11 }">
                                             营销
                                </c:if>
							</td>
							<td>${dt.feeCount }</td>
							...
							<td>
							    <c:choose>
                                <c:when test="${dateStr != null && dateStr != ''}">
                                    <a href="#" onclick="toFailAnalysis('${dt.apiAccount}',${dt.smsCategory},'${dateFormat }','${sTime}','${eTime}');">${dt.failedCount }</a>
                                </c:when>
                                <c:otherwise>
                                    <a href="#" onclick="toFailAnalysis('${dt.apiAccount}',${dt.smsCategory},'${dt.dateTime }','${sTime}','${eTime}');">${dt.failedCount }</a>
                                </c:otherwise>
                                </c:choose>
							</td>
							...
							<td>
                             <c:if test="${permission == true }">
							    <c:choose>
                                    <c:when test="${dateStr != null && dateStr != ''}">
                                         <!--<a href="/sms_send_stat/to_view/${dt.apiAccount}/${dt.smsCategory}/${dateFormat }">查看</a>-->
                                         <a href="#" onclick="toView('${dt.apiAccount}','${merchantPhone}','${businessName}',${dt.smsCategory},'${dateFormat }','${sTime}','${eTime}')">查看</a>
                                    </c:when>
							        <c:otherwise>
							            <!--<a href="/sms_send_stat/to_view/${dt.apiAccount}/${dt.smsCategory}/${dt.dateTime}">查看</a>-->
							            <a href="#" onclick="toView('${dt.apiAccount}','${merchantPhone}','${businessName}',${dt.smsCategory},'${dt.dateTime }','${sTime}','${eTime}')">查看</a>
							        </c:otherwise>
							    </c:choose>
							  </c:if>
							</td>
                        ...
                        <tr class="gradeC odd" style="font-weight:bold">
                            ...
                            <td></td>
                            <td></td>
                            <td>${smsDailyStatistics.feeCount }</td>
                            <td>${smsDailyStatistics.sendCount }</td>
                            <td>${smsDailyStatistics.successCount }</td>
                            <td>${smsDailyStatistics.failedCount }</td>
                            <td>${smsDailyStatistics.noreportCount }</td>
                            <td>
                                <fmt:formatNumber value="${smsDailyStatistics.successSendRate * 100}"
                                    pattern="0.00" maxFractionDigits="2"/>%
                            </td>
                            <td>
                                <fmt:formatNumber value="${smsDailyStatistics.failSendRate * 100}"
                                    pattern="0.00" maxFractionDigits="2"/>%
                            </td>
                            <td>
                                <fmt:formatNumber value="${smsDailyStatistics.noreportRate * 100}"
                                    pattern="0.00" maxFractionDigits="2"/>%
                            </td>
                            <c:choose>
                                <c:when test="${dateStr != null && dateStr != ''}">
                                ...
                                <c:otherwise>
                                    <td>${smsDailyStatistics.avgSendTime }</td>
                                    <td>${smsDailyStatistics.avgStatusTime   }</td>
                                    <td>
                                        <fmt:formatNumber value="${smsDailyStatistics.sdUs10sCountRate * 100}"
                                            pattern="0.00" maxFractionDigits="2"/>%
                                    </td>
                                    <td>
                                       <fmt:formatNumber value="${smsDailyStatistics.sdUs50sCountRate * 100}"
                                            pattern="0.00" maxFractionDigits="2"/>%
                                    </td>
                                    <td>
                                        <fmt:formatNumber value="${smsDailyStatistics.sdUsgt50sCountRate * 100}"
                                            pattern="0.00" maxFractionDigits="2"/>%
                                    </td>
                                    <td>
                                        <fmt:formatNumber value="${smsDailyStatistics.stBk24hCountRate * 100}"
                                            pattern="0.00" maxFractionDigits="2"/>%
                                    </td>
                                    <td>
                                        <fmt:formatNumber value="${smsDailyStatistics.stBk48hCountRate * 100}"
                                            pattern="0.00" maxFractionDigits="2"/>%
                                    </td>
                                    <td>
                                        <fmt:formatNumber value="${smsDailyStatistics.stBk72hCountRate * 100}"
                                            pattern="0.00" maxFractionDigits="2"/>%
                                    </td>
                                </c:otherwise>
                            </c:choose>
