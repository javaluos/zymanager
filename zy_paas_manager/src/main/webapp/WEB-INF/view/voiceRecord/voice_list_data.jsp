			...
			<tbody>
				...
				<c:if test="${not empty pgdata.data }">
					<c:forEach var="dt" items="${pgdata.data}" varStatus="dtindex">
						<tr class="gradeC odd">
							<td>${(pgdata.page_num-1)*pgdata.page_size + dtindex.index+1}<input type="hidden" name="callId" value="${dt.callid}"/>
							</td>
							...
							<td>
							   <c:if test="${dt.type=='1'}">回呼</c:if>
					    	   <c:if test="${dt.type=='2'}">号码卫士</c:if>
					    	   <c:if test="${dt.type=='3'}">直拨</c:if>
					    	   <c:if test="${dt.type=='4'}">语音通知</c:if>
					    	   <c:if test="${dt.type=='5'}">语音验证码</c:if>
					    	   <c:if test="${dt.type=='6'}">呼叫中心</c:if>
					    	   <c:if test="${dt.type=='7'}">多方通话</c:if>
							</td>
							<td></td>
							<!-- <td>
							</td>
							...
							<td>${dt.holdTime}</td>
							<td><a href="/voiceRecord/voice_record_content?callId=${dt.callid}">查看</a></td>
						</tr>
					</c:forEach>
