				...
				<c:if test="${not empty pgdata.data }">
					<c:forEach var="dt" items="${pgdata.data}" varStatus="dtindex">
						<tr class="gradeC odd">
							...
							<td>
							   <a onclick="toChanneltAttr('${dt.channelId}','${dt.smsCategory}','${dt.dateTime}')">查看</a>
							</td>	
						</tr>
					</c:forEach>
...
function toChanneltAttr(channelId,smsCategory,dateTime){
	window.location.href="/channel_revenue/revenue_channel_attr_list?channelId="+channelId+"&smsCategory="+smsCategory+"&dateTime="+dateTime;
}
</script>
