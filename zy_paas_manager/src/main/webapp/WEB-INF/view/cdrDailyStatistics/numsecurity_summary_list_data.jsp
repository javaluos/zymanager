				...
				<c:if test="${not empty pgdata.data }">
					<c:forEach var="dt" items="${pgdata.data}" varStatus="dtindex">
						<tr class="gradeC odd">
							...
							<td>${dt.date_time}</td>
							<td>${dt.sum_a_call_times}</td>
							<td>${dt.sum_b_holding_time }</td>
							<td>${dt.sum_fee_time}</td>
							<td>${dt.sum_b_call_suc_times}</td>
							<td>${dt.pct_call_sucdouble}%</td>
							<td>${dt.sum_b_response_times }</td>
							<td>${dt.pct_b_response_sucdouble}%</td>
							<td>${dt.avg_b_acd}</td>
						</tr>
					</c:forEach>
 					<tr class="gradeC odd">
							...
							<td></td>
							<td></td>
							<td style="font-weight: bold;"><fmt:parseNumber integerOnly="true" value="${pgdata.sumCallTimes / 2}"></fmt:parseNumber> </td>
							<td style="font-weight: bold;">${pgdata.sumHoldingTime}</td>
							<td style="font-weight: bold;">${pgdata.sumFeeTimes}</td>
							<td style="font-weight: bold;">${pgdata.sumCallSucTimes}</td>
