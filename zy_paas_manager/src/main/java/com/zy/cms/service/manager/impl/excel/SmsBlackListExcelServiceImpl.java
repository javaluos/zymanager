 ...
 *
 */
@Service("smsBlackListExcelService")
public class SmsBlackListExcelServiceImpl implements SmsBlackListExcelService {

	private static final ZyLogger logger = ZyLogger.getLogger(SmsBlackListExcelServiceImpl.class);
	...
	public WebResult exportBlackExcel(SmsBlackListQuery query, String realPath, String webUrl) {
		...
		try {
			...
			String sheetName = "短信黑名单列表"; // sheet名称
			...
			logger.info("【短信黑名单列表导出】params={savePath:" + savePath + ",downUrl=" + downUrl + "}");
			...
			if (dataList.size() > 200000) {
				logger.info("【短信黑名单列表导出】导出失败，数据超过20万行.");
				webRs.setCode(-1);
			...
			logger.error("【短信黑名单列表导出】失败 error=" + e.getMessage());
			webRs.setCode(-1);
			webRs.setMsg("导出数据失败.");
		}

		logger.info("【短信黑名单列表导出】result=" + JsonUtil.toJsonString(webRs));
		return webRs;
	}
	...
	private List<Map<String, Object>> initExcelBody(SmsBlackListQuery query) {
		...
		logger.info("【短信黑名单列表导出】查询参数 query=" + JsonUtil.toJsonString(query));
		// 定义填充数据列表
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				int i=1;
				for (SmsBlackListInfo smsBlackListInfo : list) {
					dataItem = new HashMap<String, Object>();
					
					dataItem.put("XH1",i);//序列号
					dataItem.put("XH2",smsBlackListInfo.getMobile() );//黑名单号码
					dataItem.put("XH3",smsBlackListInfo.getBusinessName() );//所属客户
					String createDate=DateUtil.formatDate(smsBlackListInfo.getCreateTime(), "yyyy-MM-dd HH:mm:ss");
					if(createDate==null){
						createDate="";
					}
			...
			logger.info("【短信黑名单列表导出】出现异常,异常是:" + e.getMessage());
		}
		return dataList;
	}
}
